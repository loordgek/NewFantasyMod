...
public class FantasyChunkGenerator extends NoiseChunkGenerator<FantasyGenSettings> {

	private static final float[] field_222576_h = Util.make(new float[25], (p_222_575_0_) -> {
		for (int i = -2; i <= 2; ++i) {
			for(int j = -2; j <= 2; ++j) {
				float f = 10 / MathHelper.sqrt((float) (i * i + j * j) + 0.2F);
				p_222_575_0_[i + 2 + (j + 2) * 5] = f;
			}
		}
	});
	...
	public FantasyChunkGenerator(IWorld worldIn, BiomeProvider biomeProviderIn, FantasyGenSettings settingsIn) {
		super(worldIn, biomeProviderIn, 4, 8, 256, settingsIn, true);
		this.randomSeed.skip(2650);
	...
	protected void fillNoiseColumn(double[] noiseColumn, int noiseX, int noiseZ) {
		...
		double d2 = 8.555149810568225D;
		double d3 = 4.544196871235463D;
		int i = -10;
		int j = 3;
